...
-- Root
local SwordSpinModule = {}

-- Functions
function SwordSpinModule.DetectHit(player: Player, hit: BasePart, velocity: Vector3)
...
function SwordSpinModule.ActivateHitbox(player: Player, region: BasePart)
	...
	region.Touched:Connect(function(hit: BasePart) 
		...
		SwordSpinModule.DetectHit(player, hit, velocity)
...
function SwordSpinModule.Main(player: Player, properties: {})
	--[[
	[PROPERTIES]
	> Combo: number
	> Origin: CFrame
	...
	if player.Character:FindFirstChild("Stunned") or player.Character:FindFirstChild("RagdollState") then return end
	if player.Character:FindFirstChild(`SwordSpinCD`) then return end
	
	Cooldown.SetEndlag(player.Character, "SwordSpin", 0.5)
	if EndLag.EndLagInterrupt(player.Character, "SwordSpin") then return end
	
	Cooldown.SetCooldown(player.Character, "SwordSpinCD", 3)
	
	-- (2) Play SwordSpinModule SFX
	...
	SwordSpinModule.ActivateHitbox(player, region)
end

return SwordSpinModule
