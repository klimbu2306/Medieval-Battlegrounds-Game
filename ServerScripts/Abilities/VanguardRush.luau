...
function VanguardRushModule.Main(player: Player, properties: {})
	...
	if player.Character:FindFirstChild(`VanguardRushCD`) then return end
		
	Cooldown.SetEndlag(player.Character, "VanguardRush", 0.25)
	if EndLag.EndLagInterrupt(player.Character, "VanguardRush") then return end
	
	Cooldown.SetCooldown(player.Character, "VanguardRushCD", 15)
	Speed.ApplySpeed(player.Character, 0, 0.25)
	
	-- (2) Play Fireball effect on the Client-side
	local VanguardRushId = `VanguardRush{time()}`
	Effects.LoadAbility:FireAllClients("VanguardRush", player, VanguardRushId, properties)

	-- (3) Setup Fireball
	local velocity = (Origin.LookVector) * SPEED 
	local Fireball = {CFrame = CFrame.new(Origin.Position, Origin.Position + velocity) * CFrame.new(0, 1, 0) + Origin.LookVector * 7.5}
	...
	-- (5) Create Vanguard Squad Hitbox
	local HitboxModel: Model = script.SquadronHitbox:Clone()
	HitboxModel:PivotTo(Fireball.CFrame)
	HitboxModel.Parent = workspace.FX
	Debris:AddItem(HitboxModel, LIFE_TIME)
	
	-- (6) Move Fireball Hitbox
	local HitTargets = {}
	...
	connection = RunService.Heartbeat:Connect(function(DT: number)
		...
		local FrontGeneral: Model = HitboxModel:FindFirstChild("FrontGeneral")
		if not FrontGeneral then
			connection:Disconnect(); connection = nil
			return
		end
		
		local hitParts = workspace:GetPartBoundsInBox(HitboxModel.FrontGeneral.Torso.CFrame, Vector3.new(15, 2, 8), FParams)
		
		if #hitParts > 0 then
			local hitParts = workspace:GetPartBoundsInRadius(HitboxModel.FrontGeneral.Torso.Position, 13, FParams)
			...
			for _, hit: BasePart in hitParts do
				...
				if humanoid and not table.find(HitTargets, humanoid) then
					...
					PlayerTag.Tag(player, target, 15)
			...
			Effects.EffectRemoving:FireAllClients("VanguardRush", VanguardRushId)
