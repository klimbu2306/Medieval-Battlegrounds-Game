...
function Counter.ActivateHitbox(player: Player, region: BasePart)
...
function Counter.Main(player: Player, properties: {})
	...
	if player.Character:FindFirstChild("Stunned") or player.Character:FindFirstChild("RagdollState") then return end
	if player.Character:FindFirstChild(`ExcaliburCD`) then return end
	
	Cooldown.SetEndlag(player.Character, "Excalibur", 0.75)
	if EndLag.EndLagInterrupt(player.Character, "Excalibur") then return end
	
	Cooldown.SetCooldown(player.Character, "ExcaliburCD", 10)
	
	local ExcaliburId = `Excalibur{time()}`
	Effects.LoadAbility:FireAllClients("Excalibur", player, ExcaliburId, {}, Origin)
	...
	IFrames.ApplyIFrames(player.Character, 0.75)
	...
	Counter.ActivateHitbox(player, region)
end
