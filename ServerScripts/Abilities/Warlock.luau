...
-- Functions
function WarlockModule.Main(player: Player, properties: {})
	--[[
	[PROPERTIES]
	> Origin: CFrame
	--]]
	...
	if player.Character:FindFirstChild(`WarlockCD`) then return end
		
	--Cooldown.SetEndlag(player.Character, "RoyalArcher", 0.25)
	if EndLag.EndLagInterrupt(player.Character, "Warlock") then return end
	
	Cooldown.SetCooldown(player.Character, "WarlockCD", 6)
	--Speed.ApplySpeed(player.Character, 0, 0.25)
	
	-- (2) Play Fireball effect on the Client-side
	local WarlockId = `Warlock{time()}`
	Effects.LoadAbility:FireAllClients("Warlock", player, WarlockId, properties)
	...
	connection = RunService.Heartbeat:Connect(function(DT: number)
		...
		if #hitParts > 0 then
			local hitParts = workspace:GetPartBoundsInRadius(Fireball.CFrame.Position, 16, FParams)
			...
			Effects.EffectRemoving:FireAllClients("Warlock", WarlockId)
