...
-- Functions
function DirtyTrick.DetectHit(player: Player, hit: BasePart, velocity: Vector3)
	...
	PlayerTag.Tag(player, target, 10)
	...
	--Effects.LoadEffect:FireAllClients("Rubble", humanoidRootPart.CFrame)
...
function DirtyTrick.ActivateHitbox(player: Player, region: BasePart)
	...
	region.Touched:Connect(function(hit: BasePart) 
		...
		-- (A) Ignore Targets of a Certain CollisionGroup
		if hit.CollisionGroup == "SupportCharacter" then return end
		...
		if player.Character:FindFirstChild("RagdollState") then return end
		...
		DirtyTrick.DetectHit(player, hit, velocity)
		table.insert(hitTargets, target)
	end)
end

function DirtyTrick.Main(player: Player, properties: {})
	--[[
	[PROPERTIES]
	> Combo: number
	> Origin: CFrame
	...
	-- (** IGNORE WHETHER PLAYER IS STUNNED, AS THIS IS AN ESCAPE COMBO MOVE)
	if player.Character:FindFirstChild("RagdollState") then return end
	if player.Character:FindFirstChild(`DirtyTrickCD`) then return end
	
	--Cooldown.SetEndlag(player.Character, "RoyalGuard", 0.5)
	if EndLag.EndLagInterrupt(player.Character, "DirtyTrick") then return end
	
	Cooldown.SetCooldown(player.Character, "DirtyTrickCD", 12)
	
	local DirtyTrickId = `DirtyTrick{time()}`
	Effects.LoadAbility:FireAllClients("DirtyTrick", player, DirtyTrickId, {}, Origin)
	
	-- (2) Play DirtyTrick SFX
	...
	-- (3) Stun the Player
	Effects.LoadEffect:FireAllClients("Highlight", player.Character, Color3.fromRGB(0, 255, 255), 0.5)
	...
	local region = Hitbox.CreateRegion(origin + origin.LookVector * 2.5, regionSize, 0.25, "MeleeHitbox", Enum.PartType.Ball)
	...
	DirtyTrick.ActivateHitbox(player, region)
end
