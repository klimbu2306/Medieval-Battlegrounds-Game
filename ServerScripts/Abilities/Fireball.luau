...
function FireballModule.Main(player: Player, properties: {})
	...
	if player.Character:FindFirstChild(`FireballCD`) then return end
		
	Cooldown.SetEndlag(player.Character, "Fireball", 1)
	if EndLag.EndLagInterrupt(player.Character, "Fireball") then return end
	
	Cooldown.SetCooldown(player.Character, "FireballCD", 4)

	-- (2) Play Fireball effect on the Client-side
	local FireballId = `Fireball{time()}`
	Effects.LoadAbility:FireAllClients("Fireball", player, FireballId, properties)

	-- (3) Setup Fireball
	local Fireball = {CFrame = Origin * CFrame.new(0, 1, 0)}
	...
	connection = RunService.Heartbeat:Connect(function(DT: number)
		...
		if #hitParts > 0 then
			...
			Effects.EffectRemoving:FireAllClients("Fireball", FireballId)
