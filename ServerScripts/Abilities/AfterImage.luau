...
local PlatformStand = require(PhysicsClient:WaitForChild("PlatformStand"))

-- Root
local Lunge = {}

-- Functions
function Lunge.DetectHit(player: Player, hit: BasePart, velocity: Vector3)
...
function Lunge.ActivateHitbox(player: Player, region: BasePart)
	...
	region.Touched:Connect(function(hit: BasePart) 
		...
		-- (0) Nullify Hitbox Touched if Target has I-Frames + Trigger Possible Counterattack Effects
		...
		Lunge.DetectHit(player, hit, velocity)
...
function Lunge.HitGround(player: Player, Origin: CFrame)
	...
	local regionMagnitude = 20
	local regionSize = Vector3.one * regionMagnitude

	local region = Hitbox.CreateRegion(player.Character.PrimaryPart.CFrame, regionSize, 0.25, "MeleeHitbox", Enum.PartType.Ball)
	...
	Lunge.ActivateHitbox(player, region)
end

function Lunge.Main(player: Player, properties: {})
	--[[
	[PROPERTIES]
	> Combo: number
	> Origin: CFrame
	...
	if player.Character:FindFirstChild("Stunned") or player.Character:FindFirstChild("RagdollState") then return end
	if player.Character:FindFirstChild(`AfterImageCD`) then return end
	
	Cooldown.SetEndlag(player.Character, "AfterImage", 0.75)
	if EndLag.EndLagInterrupt(player.Character, "AfterImage") then return end
	
	Cooldown.SetCooldown(player.Character, "AfterImageCD", 5)
	...
	local LungeTowards = Origin.Position + Origin.LookVector * 70 + Vector3.new(0, 0, 0)
	
	Effects.LoadEffect:FireAllClients("Highlight", player.Character, Color3.fromRGB(255, 200, 0), 1.25)
	IFrames.ApplyIFrames(player.Character, 1)
	Speed.ApplySpeed(player.Character, 0, 0.5)
	
	Knockback.ApplyFixedForce(player, player.Character, LungeTowards, LUNGE_TIME, {MaxForce = 8e4, MaxVelocity = 8e4})
	Effects.LoadAbility:FireAllClients("AfterImage", player)
end
