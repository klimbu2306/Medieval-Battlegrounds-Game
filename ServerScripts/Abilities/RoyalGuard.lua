...
-- Functions
function RoyalGuard.DetectHit(player: Player, hit: BasePart, velocity: Vector3)
	...
	Effects.LoadEffect:FireAllClients("Crater", {Rotation = -3, Radius = 5.5, Duration = 0.25}, humanoidRootPart.CFrame)
...
function RoyalGuard.ActivateHitbox(player: Player, region: BasePart)
	...
	region.Touched:Connect(function(hit: BasePart) 
		...
		RoyalGuard.DetectHit(player, hit, velocity)
		table.insert(hitTargets, target)
	end)
end

function RoyalGuard.Main(player: Player, properties: {})
	--[[
	[PROPERTIES]
	> Combo: number
	> Origin: CFrame
	...
	if player.Character:FindFirstChild("Stunned") or player.Character:FindFirstChild("RagdollState") then return end
	if player.Character:FindFirstChild(`RoyalGuardCD`) then return end
	
	--Cooldown.SetEndlag(player.Character, "RoyalGuard", 0.5)
	if EndLag.EndLagInterrupt(player.Character, "RoyalGuard") then return end
	
	Cooldown.SetCooldown(player.Character, "RoyalGuardCD", 8)
	
	local RoyalGuardId = `RoyalGuard{time()}`
	Effects.LoadAbility:FireAllClients("RoyalGuard", player, RoyalGuardId, {}, Origin)
	
	-- (2) Play RoyalGuard SFX
	...
	RoyalGuard.ActivateHitbox(player, region)
end
