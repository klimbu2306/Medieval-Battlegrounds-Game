...
function FireballModule.Main(player: Player, properties: {})
	...
	if player.Character:FindFirstChild(`EnergySlashCD`) then return end
		
	Cooldown.SetEndlag(player.Character, "EnergySlash", 0.25)
	if EndLag.EndLagInterrupt(player.Character, "EnergySlash") then return end
	
	Cooldown.SetCooldown(player.Character, "EnergySlashCD", 4)
	Speed.ApplySpeed(player.Character, 0, 0.25)
	
	-- (2) Play Fireball effect on the Client-side
	local EnergySlashId = `EnergySlash{time()}`
	Effects.LoadAbility:FireAllClients("EnergySlash", player, EnergySlashId, properties)

	-- (3) Setup Fireball
	local velocity = (Origin.LookVector) * SPEED 
	local Fireball = {CFrame = CFrame.new(Origin.Position, Origin.Position + velocity) * CFrame.new(0, 1, 0)}
	local startTime = time()
	
	-- (4) Setup Fireball Hitbox Parameters
	local FParams = OverlapParams.new()
	FParams.FilterType = Enum.RaycastFilterType.Exclude
	FParams.FilterDescendantsInstances = {workspace.FX, player.Character, workspace.Map, workspace.Hitbox, workspace.TycoonHitbox.Floor}
	...
	connection = RunService.Heartbeat:Connect(function(DT: number)
		...
		local hitParts = workspace:GetPartBoundsInBox(Fireball.CFrame, Vector3.new(12, 2, 5), FParams)
		...
		if #hitParts > 0 then
			...
			Effects.EffectRemoving:FireAllClients("EnergySlash", EnergySlashId)
