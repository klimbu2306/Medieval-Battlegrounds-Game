...
function Trail.Raycast(origin: CFrame): RaycastResult
	...
	raycastParams.FilterDescendantsInstances = {workspace.Map, workspace.TycoonHitbox.Floor}
...
function Trail.Main(...)
	...
	for i = 0, CRATER_PIECES - 1 do
		...
		local Detect = workspace:Raycast(Part.Position + Vector3.new(0, 2.5, 0), Part.Position + UpVector * -10, NParams)
		if Detect then
